...
def run_provider_show():
    ...
    def _fmt(provider_key: str, lc_model_key: str, _: str, lc_backend_key: str) -> str:
        ...
        if backend == "langchain":
            ...
            lc_backend = (
                cfg.get(lc_backend_key) or cfg.get("LANGCHAIN_BACKEND") or "ollama"
            ).lower()
...
def run_provider_set(
    target: str, backend: str, model: str | None, lc_backend: str | None = None
):
    ...
    if backend == "langchain":
        ...
        if b == "deepseek" and not (
            os.getenv("DEEPSEEK_API_KEY") or _read_env_value("DEEPSEEK_API_KEY")
        ):
            print(
                "  [warn] DEEPSEEK_API_KEY not set. Run: provider key set deepseek <key>"
            )
    else:
        if model:
...
def run_provider_key_set(provider: str, value: str):
    ...
    if provider not in _PROVIDER_KEY_ENV:
        raise ValueError(
            f"Unknown provider '{provider}'. Available: {', '.join(_PROVIDER_KEY_ENV)}"
        )
...
def register_provider_commands(provider_app: typer.Typer) -> None:
    from src.interfaces.cli.enums import ProviderTarget, LLMBackend
    ...
    def provider_set(
        target: ProviderTarget = typer.Argument(
            ...,
            help="Which provider to change: 'llm' (form Q&A) or 'eval' (job evaluation)",
        ),
        backend: LLMBackend = typer.Argument(
            ..., help="Backend to use: claude or langchain"
        ),
        model: Optional[str] = typer.Option(
            None,
            "--model",
            help="Model name (e.g. claude-haiku-4-5-20251001 or llama3.1:8b)",
        ),
        lc_backend: Optional[str] = typer.Option(
            None, "--backend", help="LangChain backend: ollama (default) or deepseek"
        ),
    ):
