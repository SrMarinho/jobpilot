...
from src.interfaces.cli.engage.logic import capture_metrics, _ALL_METRICS
...
def register_profile_capture_command(app: typer.Typer) -> None:
    ...
    def cmd_capture(
        ctx: typer.Context,
        force: bool = typer.Option(False, "--force", help="Recaptura mesmo se já capturou hoje"),
        ssi: bool = typer.Option(False, "--ssi", help="Captura apenas SSI"),
        views: bool = typer.Option(False, "--views", help="Captura apenas profile views"),
        appearances: bool = typer.Option(False, "--appearances", help="Captura apenas aparições em pesquisa"),
    ):
        ...
        selected = frozenset(
            m for m, flag in [("ssi", ssi), ("views", views), ("appearances", appearances)] if flag
        )
