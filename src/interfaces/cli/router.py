...
from src.interfaces.cli.profile.appearances.command import register_profile_appearances_commands
...
from src.interfaces.cli.misc.command import register_misc_commands, register_test_apply_command
...
from src.interfaces.cli.provider.command import register_provider_commands, register_key_commands
...
def register(app: typer.Typer) -> None:
    ...
    _add_group(jobs_app, "searches", "Rotating saved-searches (connect/apply)", register_searches_commands)
    _add_group(jobs_app, "skills", "Missing skills detected during job evaluation", register_skills_commands)
    _add_group(jobs_app, "answers", "Cached form answers (form_answers.json)", register_answers_commands)
    ...
    _add_group(profile_app, "skills", "Add/delete/sync LinkedIn profile skills", register_profile_skills_commands)
    _add_group(profile_app, "views", "Track & analyze 90-day profile views", register_profile_views_commands)
    _add_group(profile_app, "ssi", "Social Selling Index: show score + history", register_profile_ssi_commands)
    _add_group(profile_app, "appearances", "Search appearances: count + trend", register_profile_appearances_commands)
    ...
    _add_group(config_app, "telegram-topics", "Telegram topic sessions: setup/list/reset", register_telegram_topics_commands)
    _add_group(config_app, "goals", "Weekly goals (applications/connections/posts/comments)", register_goals_commands)
    _add_group(config_app, "db", "Postgres backend: check/init/migrate/status", register_db_commands)
    provider_app = _add_group(config_app, "provider", "LLM provider settings", register_provider_commands)
