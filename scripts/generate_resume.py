...
name_style = ParagraphStyle('Name', parent=styles['Heading1'], fontSize=22, textColor=PRIMARY,
                            spaceAfter=2, fontName='Helvetica-Bold', alignment=TA_LEFT, leading=24)
job_title_header = ParagraphStyle('JTH', parent=styles['Normal'], fontSize=12, textColor=ACCENT,
                                  spaceAfter=4, fontName='Helvetica-Bold')
contact_style = ParagraphStyle('Contact', parent=styles['Normal'], fontSize=9, textColor=LIGHT,
                               spaceAfter=10, fontName='Helvetica')
section_heading = ParagraphStyle('SH', parent=styles['Heading2'], fontSize=11, textColor=PRIMARY,
                                 spaceAfter=4, spaceBefore=10, fontName='Helvetica-Bold', leading=13)
summary_style = ParagraphStyle('Summary', parent=styles['BodyText'], fontSize=9.5, textColor=TEXT,
                               spaceAfter=6, leading=12.5, fontName='Helvetica', alignment=TA_JUSTIFY)
job_position_style = ParagraphStyle('JP', parent=styles['Normal'], fontSize=10.5, textColor=SECONDARY,
                                    spaceAfter=1, fontName='Helvetica-Bold')
company_period_style = ParagraphStyle('CP', parent=styles['Normal'], fontSize=9, textColor=LIGHT,
                                      spaceAfter=3, fontName='Helvetica-Oblique')
bullet_style = ParagraphStyle('Bullet', parent=styles['BodyText'], fontSize=9, textColor=TEXT,
                              spaceAfter=2, leading=12, fontName='Helvetica',
                              leftIndent=12, bulletIndent=2)
project_name_style = ParagraphStyle('PN', parent=styles['Normal'], fontSize=10, textColor=SECONDARY,
                                    spaceAfter=1, fontName='Helvetica-Bold')
project_meta_style = ParagraphStyle('PM', parent=styles['Normal'], fontSize=8.5, textColor=LIGHT,
                                    spaceAfter=2, fontName='Helvetica-Oblique')
skill_label_style = ParagraphStyle('SL', parent=styles['Normal'], fontSize=9, textColor=PRIMARY,
                                   fontName='Helvetica-Bold', leading=12)
skill_value_style = ParagraphStyle('SV', parent=styles['Normal'], fontSize=9, textColor=TEXT,
                                   fontName='Helvetica', leading=12)
edu_style = ParagraphStyle('Edu', parent=styles['Normal'], fontSize=9.5, textColor=TEXT,
                           spaceAfter=3, fontName='Helvetica', leading=12)

content = []

content.append(Paragraph("MATHEUS MARINHO", name_style))
content.append(Paragraph("Desenvolvedor Full-Stack | Python &amp; TypeScript", job_title_header))
content.append(Paragraph(
    "Teresina, PI &nbsp;&bull;&nbsp; (86) 98102-2333 &nbsp;&bull;&nbsp; temarinho76@gmail.com &nbsp;&bull;&nbsp; github.com/SrMarinho",
    contact_style
))
content.append(HRFlowable(width="100%", thickness=1.5, color=PRIMARY, spaceBefore=0, spaceAfter=8))

content.append(Paragraph("EXPERI&Ecirc;NCIA PROFISSIONAL", section_heading))
content.append(HRFlowable(width="100%", thickness=0.5, color=LIGHT, spaceBefore=0, spaceAfter=4))
content.append(Paragraph("Desenvolvedor Full-Stack Pleno", job_position_style))
content.append(Paragraph("Nazaria - Distribuidora Farmac&ecirc;utica &nbsp;|&nbsp; Setembro 2022 - Presente",
...
for p in projects:
    content.append(Paragraph("<b>" + p['name'] + "</b> &nbsp;<font color='#3182ce'>&bull;</font>&nbsp; " + p['tech'],
                             project_name_style))
...
skills = [
    ...
    ("ETL &amp; Automa&ccedil;&atilde;o", "ETL, RPA, Selenium, Web Scraping, Multi-threading"),
    ("Metodologias", "Scrum, Agile, TDD"),
    ("Idiomas", "Portugu&ecirc;s (Nativo), Ingl&ecirc;s (Intermedi&aacute;rio - leitura t&eacute;cnica avan&ccedil;ada)"),
]

skills_data = [[Paragraph(label, skill_label_style), Paragraph(value, skill_value_style)]
...
content.append(HRFlowable(width="100%", thickness=0.5, color=LIGHT, spaceBefore=0, spaceAfter=4))
content.append(Paragraph("<b>P&oacute;s-gradua&ccedil;&atilde;o em Engenharia de Software</b> &nbsp;<font color='#3182ce'>(Cursando)</font>", edu_style))
content.append(Paragraph("PUC Minas", company_period_style))
content.append(Spacer(1, 0.04 * inch))
content.append(Paragraph("<b>Bacharelado em Ci&ecirc;ncia da Computa&ccedil;&atilde;o</b>", edu_style))
content.append(Paragraph("Universidade Est&aacute;cio de S&aacute;", company_period_style))
