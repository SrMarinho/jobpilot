...
def _resolve_apply_url_task(
    ...
    if url:
        ...
        start_page = 1
        search_params = {
            "keywords": " ".join(keywords), "date_posted": date_posted,
            "workplace": workplace, "location": location, "experience": experience,
        }
        print(f"Using search: keywords='{' '.join(keywords)}'" + (f", date_posted={date_posted}" if date_posted else "") + (f", workplace={workplace}" if workplace else ""))
    else:
...
def _search_params_dict(
    keywords: list[str] | None, date_posted: str | None,
    workplace: str | None, location: str | None, experience: str | None,
) -> dict:
    return {
        "keywords": " ".join(keywords) if keywords else None, "date_posted": date_posted,
        "workplace": workplace, "location": location, "experience": experience,
    }
...
def apply(
    ...
    keywords: Optional[List[str]] = typer.Option(None, "--keywords", "-k", help="Search keywords (repeat: --keyword python --keyword node)"),
...
def connect(
    ...
    keywords: Optional[List[str]] = typer.Option(None, "--keywords", "-k", help="Search keywords (repeat: --keyword tech --keyword recruiter)"),
    ...
    if url:
        ...
        resolved_start_page = start_page or 1
        print(f"Using search: keywords='{' '.join(keywords)}'" + (f", network={k_network}" if k_network else ""))
    else:
        saved_kw = saved.get("keywords") if isinstance(saved, dict) else None
        saved_keywords: list[str] | None = None
        if saved_kw:
            if isinstance(saved_kw, str):
                saved_keywords = saved_kw.split()
            else:
                saved_keywords = saved_kw
        if saved_keywords:
    ...
    final_keywords_str = " ".join(keywords) if keywords else (saved.get("keywords") if isinstance(saved, dict) else None)
